...
--
-- Host: 127.0.0.1
-- Tempo de geração: 11-Jun-2025 às 13:56
...
INSERT INTO `cartao` (`NUMERO_CARTAO`, `NOME_RESPONSAVEL`, `DATA_VALIDADE`, `CVV`, `ENDERECO_COBRANCA`, `SENHA`, `BANDEIRA`) VALUES
(2147483647, 'Sthefani de Moraes Meurer', '09/27', 154, 'av sao diego numero 123 bairro santo antonio fraiburgo sc', 1245, 'mastrcard');
...
INSERT INTO `clientes` (`ID_CLIENTE`, `PRIMEIRO_NOME`, `SOBRENOME`, `EMAIL`, `GENERO`, `NOME_EMPRESA`) VALUES
...
CREATE TABLE `conta` (
  ...
  `LIMITE_SAQUE` text DEFAULT '200 REAIS',
...
--
ALTER TABLE `clientes`
  MODIFY `ID_CLIENTE` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=701;
...
--
ALTER TABLE `pedidos`
  ADD CONSTRAINT `pedidos_ibfk_1` FOREIGN KEY (`numeroCartao`) REFERENCES `cartao` (`NUMERO_CARTAO`),
  ADD CONSTRAINT `pedidos_ibfk_2` FOREIGN KEY (`idCliente`) REFERENCES `clientes` (`ID_CLIENTE`);
COMMIT;
